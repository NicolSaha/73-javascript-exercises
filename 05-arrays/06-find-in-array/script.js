...
(function () {
  ...
  // peopleMatch = people.filter(
  //   (x) => x.firstname == "Jean" && x.lastname == "Dupont" );

  const peopleMatchIndex = people.findIndex(function (x) {
    if (x.firstname == "Jean" && x.lastname == "Dupont") {
      return true;
    }
  });

  console.log(peopleMatchIndex);
  console.log(people[peopleMatchIndex].email);

  alert(
    `Index = ${peopleMatchIndex}, Email = ${people[peopleMatchIndex].email}`
  );
})();
